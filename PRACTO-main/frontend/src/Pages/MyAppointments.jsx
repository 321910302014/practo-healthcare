                ...
                <tr key={index} className='border-b hover:bg-gray-50 transition-all'>
                  ...
                  <td className='px-4 py-4 text-center'>
                    <div className='flex flex-col gap-2 min-w-32'>
                      {item.videoConsultation && !item.cancelled && !item.isCompleted && (
                        <button
                          onClick={() => navigate(`/video-call/${item._id}`)}
                          className='text-white bg-green-600 hover:bg-green-700 border border-green-600 px-4 py-1.5 rounded-md text-xs font-semibold transition-all duration-300'
                        >
                          📹 Join Video Call
                        </button>
                      )}
