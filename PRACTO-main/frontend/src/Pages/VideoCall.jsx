...
const VideoCallContent = () => {
    ...
    const joinRoom = async () => {
        try {
            const isRealAppointment = /^[0-9a-fA-F]{24}$/.test(appointmentId || "");

            let authToken = null;
            let userName = userData?.name || "Patient";

            if (isRealAppointment && token) {
                // Booked consultation: join the room dedicated to this appointment
                ...
                // Instant consultation (no booked appointment): shared demo room
                ...
                const { data } = await axios.post(`${backendUrl}/api/100ms/generate-token`, {
                    room_id: roomId,
                    user_id: userData?._id || `user-${Date.now()}`,
                    role: "broadcaster"
                });
                if (!data.token) {
