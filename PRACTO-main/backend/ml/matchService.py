...
def build_index():
    ...
    if texts:
        print(f"🔄 Encoding {len(texts)} doctor profiles...")
        embeddings = model.encode(texts, convert_to_numpy=True, show_progress_bar=True)
        print(f"✅ Encoding complete. Shape: {embeddings.shape}")
        
        print("🔄 Normalizing embeddings (using stable Numpy)...")
        ...
        doctor_ids = [str(d["_id"]) for d in docs]

    # Build speciality embeddings
    specialities = list(set([d.get("speciality", "") for d in docs if d.get("speciality")]))
    if specialities:
        print(f"🔄 Encoding {len(specialities)} specialities...")
        speciality_embeddings = model.encode(specialities, convert_to_numpy=True)
        print("✅ Speciality encoding complete.")
        
        print("🔄 Normalizing speciality embeddings (using stable Numpy)...")
        spec_norm = np.linalg.norm(speciality_embeddings, axis=1, keepdims=True)
        speciality_embeddings = speciality_embeddings / (spec_norm + 1e-10)
        print("✅ Normalization complete.")
        
        print("🔄 Building speciality index...")
        speciality_index = faiss.IndexFlatIP(EMBED_DIM)
        speciality_index.add(speciality_embeddings.astype('float32'))
...
def match():
    ...
    query_embedding = query_embedding / (q_norm + 1e-10)
