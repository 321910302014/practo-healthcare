...
// Doctor list with optional insurance filtering + ratings + review count
const doctorList = async (req, res) => {
  try {
    ...
    // Add average rating and number of reviews
    const doctorsWithRatings = await Promise.all(
      doctors.map(async (doc) => {
        ...
        const avgRating = reviewsCount > 0
          ...
          : 0;
        return { ...doc._doc, rating: avgRating, reviewsCount };
      })
    );
